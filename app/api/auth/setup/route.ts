...
export async function POST(req:Request){
 try{
  ...
  await db.transaction(async tx=>{
   ...
   await tx.insert(records).values({id:sid,kind:'shops',shopId:sid,data:JSON.stringify({id:sid,name:p.data.shopName,address:'',phone:'',country:'',currency:'USD',locale:'en-US',timezone:'UTC',taxName:'Sales tax',taxRate:0,dateFormat:'MM/DD/YYYY',active:true}),createdAt:n,updatedAt:n})
   for (const row of defaultShopRecords(sid, p.data.shopName, n)) {
    await tx.insert(records).values({ id: row.id, kind: row.kind, shopId: sid, data: JSON.stringify(row.data), createdAt: n, updatedAt: n })
   }
   await tx.insert(auditLog).values({actor:uid,action:'setup',entity:'shop',entityId:sid,detail:'Created workspace with booking, capacity and integration defaults',createdAt:n})
  })
