...
export async function POST(request: Request) {
  ...
  try {
    const response = await fetch(`https://vpic.nhtsa.dot.gov/api/vehicles/DecodeVinValuesExtended/${encodeURIComponent(parsed.data.vin)}?format=json`, { signal: AbortSignal.timeout(12000), headers: { accept: 'application/json', 'user-agent': 'AutoGaragify/1.0 VIN decoder' }, cache: 'no-store' })
